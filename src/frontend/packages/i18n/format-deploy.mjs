...
fs.readdirSync(folderPath).map((language) => {
  ...
  jsonI18n[language] = jsonKeyMessage;
});
...
// Validate all source files before writing individual locale dictionaries.
fs.mkdirSync(output, { recursive: true });
Object.entries(jsonI18n).forEach(([language, translation]) => {
  fs.writeFileSync(
    path.join(output, `${language}.json`),
